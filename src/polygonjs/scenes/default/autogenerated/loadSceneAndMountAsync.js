import {loadSceneData_default} from './loadSceneData.js?t=1648496238398';
...
export const loadSceneAndMountAsync_default = async function (options) {
	...
	const moduleNames = loadModules ? ["SVGLoader"] : [];
	const modulePromises = moduleNames.map((moduleName) => import('./modules/' + moduleName + (options.moduleImportSuffix || '')));
	const promises = [import('./loadSceneFromSceneData.js?t=1648496238398'), loadSceneData_default({onProgress, sceneDataRoot}), ...modulePromises];
